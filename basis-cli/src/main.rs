...
enum Command {
    ...
    Check {
        ...
        #[arg(default_value = ".")]
        path: PathBuf,
        /// Which axes to check (default: all). Options: placement, values, completeness, purity, contracts
...
fn run_check(
    ...
    if run("contracts") {
        ...
        let hits = check::contracts::check_contracts(spec, path, &registry);
        for v in &hits {
            violations.push(check::output::UnifiedViolation::from(v));
        }
    }
    ...
    match cli.command {
        ...
        Command::Check {
            ...
            if format == "json" {
                ...
                if run("contracts") {
                    let violations =
                        check::contracts::check_contracts(&spec, &path, &registry);
                    for v in &violations {
                        eprintln!("{v}\n");
                    }
                    total_violations += violations.len();
