...
/// Check if a line or block contains an early-return pattern that signals rejection.
fn is_early_return(text: &str) -> bool {
...
fn scan_contracts(content: &str) -> Vec<FunctionContract> {
    ...
    while i < lines.len() {
        ...
        let is_constructor = fn_name == "new" || fn_name == "from";
        ...
        // Extract parameter names (skip self variants)
        ...
        let mut body_start = end_idx + 1;
        for j in end_idx..lines.len().min(end_idx + 5) {
            if lines[j].contains('{') {
                body_start = j + 1;
                break;
            }
        ...
        for j in body_start..lines.len().min(body_start + 20) {
            ...
            // assert!( ... )
            // debug_assert!( ... )
            // if <cond> { return Err( ... ) }  (single or multi-line)
            // let ... = ... else { return ... }
            let is_single_line_guard = line_trimmed.starts_with("assert!(")
                || line_trimmed.starts_with("assert_eq!(")
                || line_trimmed.starts_with("assert_ne!(")
                || line_trimmed.starts_with("debug_assert!(")
                || (line_trimmed.starts_with("if ")
                    && is_early_return(line_trimmed))
                || (line_trimmed.starts_with("let ") && line_trimmed.contains("else {"));

            if is_single_line_guard {
                for param in &params {
            ...
            } else if line_trimmed.starts_with("if ") {
                // Multi-line if guard: look ahead for return/bail/panic
                let mut guard_block = line_trimmed.to_string();
                let lookahead_end = (j + 5).min(lines.len());
                for k in (j + 1)..lookahead_end {
                    let next = lines[k].trim();
                    guard_block.push(' ');
                    guard_block.push_str(next);
                    if next.starts_with("}") {
                        break;
                    }
                }
                if is_early_return(&guard_block) {
                    for param in &params {
                        if guard_block.contains(param.as_str())
                            && !guarded_params.contains(param)
                        {
                            guarded_params.push(param.clone());
                        }
